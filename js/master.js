const lemonade = {
  ...
  weather: ['mdi mdi-weather-lightning-rainy mdi-36px',
    'mdi mdi-weather-cloudy mdi-36px',
    'mdi mdi-weather-partlycloudy mdi-36px',
    'mdi mdi-weather-sunny mdi-36px',
  ],
  emotionBank: ['mdi mdi-emoticon-dead mdi-36px',
    'mdi mdi-emoticon-sad mdi-36px',
    'mdi mdi-emoticon-neutral mdi-36px', 'mdi mdi-emoticon mdi-36px',
    'mdi mdi-emoticon-excited mdi-36px',
    'mdi mdi-emoticon-cool mdi-36px',
  ],
  diagnostic: (signs) => {
    console.debug(
      `${lemonade.signsVar.valueAsNumber
      } signs market: ${lemonade.stringRound(signs)
      } Cost: $${lemonade.stringRound(lemonade.priceVar.valueAsNumber)
      } Weather ${lemonade.todayWeatherVariant
      } Sold ${lemonade.sold} cups`,
    );
  },
  whenLoaded: () => {
    lemonade.pourIn();
  ...
  average: () => ((lemonade.allTimeProfit - lemonade.dailyProfit) /
    lemonade.day),
  determineConfidence: () => {
    if (lemonade.allTimeProfit < 10) {
      ...
      lemonade.confidence = Math.round(((lemonade.dailyProfit -
        lemonade.average()) / 20) + 2);
  ...
  newGame: () => {
    ...
    lemonade.emotion.className = lemonade.emotionBank[3];
    lemonade.confidence = 3;
    lemonade.cupsVar.valueAsNumber = 150;
  ...
  nextDay: () => {
    ...
    lemonade.expenses = lemonade.twoDecimals(
      (lemonade.signs * lemonade.signCost) + (lemonade.cups *
        lemonade.cupCost));
    if (lemonade.expenses > lemonade.allTimeProfit) {
      ...
      if (lemonade.sold > 0 && lemonade.sold < lemonade.cups) {
        lemonade.bar.style.width =
          `${(lemonade.sold / lemonade.cups) * 100}%`;
      ...
      lemonade.profits = lemonade.twoDecimals(lemonade.sold *
        lemonade.price);
      lemonade.dailyProfit = lemonade.twoDecimals(lemonade.profits -
        lemonade.expenses);
  ...
  displayUpdate: () => {
    ...
    lemonade.cupsSoldDisplayVar.innerText =
      `${lemonade.sold} Cups | $${lemonade.stringRound(lemonade.dailyProfit)}`;
    lemonade.grandTotalDisplayVar.innerText =
      `$${lemonade.stringRound(lemonade.allTimeProfit)}`;
  },
  clearToast: () => {
    lemonade.toastVar = '';
  },
